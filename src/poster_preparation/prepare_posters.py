...
def sanitize_filename(name: str) -> str:
    """
    Convert movie name to safe filename by removing accents and special characters.
    
    Args:
        name: Movie name with potential special characters
        
    Returns:
        Safe filename string with ASCII characters only
    """
    ...
    def process_all_posters(self):
        ...
        for i, filename in enumerate(poster_files, 1):
            ...
            try:
                ...
                if region is None:
                    print(f"  ✗ No title detected - marking as failed")
        ...
        # If there are failures, offer to get replacements
        if failed:
            print(f"⚠ {len(failed)} posters failed processing.")
            print("  Run with --replace-failed to get replacement movies from the same lists\n")
    
    def replace_failed_posters(self, movie_list_file: str = "list/movie_list.txt"):
        """
        Replace failed posters with new movies from the same source lists.
        Maintains 100 movies per list by replacing failures with unused movies.
        
        Args:
            movie_list_file: Path to the movie list file
        """
        print("\n" + "=" * 60)
        print("REPLACING FAILED POSTERS")
        print("=" * 60 + "\n")
        
        list_dir = "list"
        failed_file = os.path.join(list_dir, "failed_processing.txt")
        
        # Check if failed list exists
        if not os.path.exists(failed_file):
            print("✓ No failed posters to replace")
            return
        
        # Read failed posters
        with open(failed_file, 'r', encoding='utf-8') as f:
            failed_posters = [line.strip() for line in f if line.strip()]
        
        if not failed_posters:
            print("✓ No failed posters to replace")
            return
        
        print(f"Found {len(failed_posters)} failed posters to replace\n")
        
        # Extract movie names from poster filenames (remove year and extension)
        import re
        def extract_movie_key(filename):
            # Remove extension
            name = os.path.splitext(filename)[0]
            # Remove year pattern at the end
            name = re.sub(r'_\d{4}$', '', name)
            return name
        
        failed_movie_keys = {extract_movie_key(p) for p in failed_posters}
        
        # Read sampled list files to determine which list each failed movie came from
        failed_by_list = {}  # {list_number: [failed_movies]}
        
        for list_file in os.listdir(list_dir):
            if list_file.startswith('sampled_list_') and list_file.endswith('.txt'):
                # Extract list number
                match = re.match(r'sampled_list_(\d+)\.txt', list_file)
                if not match:
                    continue
                ...
                # Read sampled movies from this list
                with open(list_path, 'r', encoding='utf-8') as f:
                    for line in f:
                        line = line.strip()
                        if line and not line.startswith('#'):
                            # Parse movie (format: "Title (Year)")
                            movie_match = re.match(r'(.+?)\s*\((\d{4})\)', line)
                            if movie_match:
                                title = movie_match.group(1).strip()
                                movie_key = title.replace(' ', '_')
                                
                                # Check if this movie failed
                                if movie_key in failed_movie_keys:
                                    if list_num not in failed_by_list:
                                        failed_by_list[list_num] = []
                                    failed_by_list[list_num].append((title, movie_match.group(2)))
        
        if not failed_by_list:
            print("⚠ Warning: Could not determine which list failed posters came from")
            print("  Sampled list files may be missing. Run --sample first.\n")
            return
        
        print("Failed posters by list:")
        for list_num, movies in failed_by_list.items():
            print(f"  List {list_num}: {len(movies)} failure(s)")
        print()
        
        # Read movie list to get available replacements per list
        ...
        # Parse lists and build available movie pool per list
        current_list = None
        list_movies = {}  # {list_number: [(title, year)]}
        all_movie_keys = set()  # Track all movies to avoid duplicates
        
        # First pass: collect all movie keys
        for list_file in os.listdir(list_dir):
            if list_file.startswith('sampled_list_') and list_file.endswith('.txt'):
                list_path = os.path.join(list_dir, list_file)
                with open(list_path, 'r', encoding='utf-8') as f:
                    for line in f:
                        ...
                        if line and not line.startswith('#'):
                            ...
                            if movie_match:
                                title = movie_match.group(1).strip()
                                movie_key = title.replace(' ', '_')
                                all_movie_keys.add(movie_key)
        
        # Second pass: build available replacements per list
        for line in lines:
            line = line.strip()
            if line.startswith('# List'):
                # Extract list number
                match = re.match(r'# List (\d+):', line)
                if match:
                    current_list = int(match.group(1))
                    list_movies[current_list] = []
            elif line and not line.startswith('=') and not line.startswith('#') and current_list is not None:
                # Parse movie (format: "Title (Year)")
                ...
                if match:
                    ...
                    title = title.strip()
                    movie_key = title.replace(' ', '_')
                    # Only add if not already in sampled lists
                    if movie_key not in all_movie_keys:
                        list_movies[current_list].append((title, year))
        
        print("Available replacement movies per list:")
        for list_num, movies in list_movies.items():
            print(f"  List {list_num}: {len(movies)} unused movies available")
        print()
        
        # Get replacements from the SAME list each failed movie came from
        import random
        replacement_movies = []
        
        for list_num, failed_movies in failed_by_list.items():
            num_needed = len(failed_movies)
            available = list_movies.get(list_num, [])
            
            if not available:
                print(f"⚠ Warning: No unused movies available in List {list_num}")
                print(f"  Cannot replace {num_needed} failed movie(s) from this list")
                continue
            
            # Take random replacements from the same list
            num_to_take = min(len(available), num_needed)
            selected = random.sample(available, num_to_take)
            replacement_movies.extend([(list_num, title, year) for title, year in selected])
            
            print(f"✓ Selected {num_to_take} replacement(s) from List {list_num}")
            
            if num_to_take < num_needed:
                print(f"  ⚠ Warning: Only {num_to_take} of {num_needed} replacements available")
        
        if not replacement_movies:
            print("\n✗ No replacement movies available")
            return
        
        # Display replacements
        print(f"\n✓ Found {len(replacement_movies)} replacement movie(s):")
        for list_num, title, year in replacement_movies:
            print(f"  [List {list_num}] {title} ({year})")
        
        # Append to movie list under appropriate list headers
        replacements_by_list = {}
        for list_num, title, year in replacement_movies:
            if list_num not in replacements_by_list:
                replacements_by_list[list_num] = []
            replacements_by_list[list_num].append((title, year))
        
        with open(movie_list_file, 'r', encoding='utf-8') as f:
            content = f.read()
        
        # Append replacements to their respective list sections
        new_content = content
        for list_num, movies in replacements_by_list.items():
            # Find the list section
            list_marker = f"# List {list_num}:"
            if list_marker in new_content:
                # Find the next list marker or end of file
                list_start = new_content.find(list_marker)
                next_list_start = new_content.find("\n# List ", list_start + 1)
                
                if next_list_start == -1:
                    # Last list, append at the end
                    insert_pos = len(new_content)
                else:
                    # Insert before next list
                    insert_pos = next_list_start
                
                # Add replacements
                replacement_text = "\n# [REPLACEMENTS]\n"
                for title, year in movies:
                    replacement_text += f"{title} ({year})\n"
                
                new_content = new_content[:insert_pos] + replacement_text + new_content[insert_pos:]
        
        # Write updated content
        with open(movie_list_file, 'w', encoding='utf-8') as f:
            f.write(new_content)
        
        print(f"\n✓ Added {len(replacement_movies)} replacement(s) to {movie_list_file}")
        print("  Next steps:")
        print("  1. Run --download to download replacement posters")
...
Examples:
  ...
  # Replace failed posters with new movies from the same lists
  python prepare_posters.py --replace-failed
  ...
  python prepare_posters.py --sample my_movies.txt
    ...
    parser.add_argument('--replace-failed', nargs='?', const='list/movie_list.txt', metavar='MOVIE_LIST',
    ...
    if args.replace_failed:
        preparer.replace_failed_posters(args.replace_failed)
        return
